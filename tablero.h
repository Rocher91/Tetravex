 ...
 * Llama a inicializar_azar() para que el tablero sea diferente cada vez.			OK
 * Pide al usuario el tamaño del tablero (verificando que el valor sea 				OK
 * correcto). Inicializa los campos size y tipo del tablero (a TABLERO_SIMPLE).		OK
 * Inicializa los campos max_f y max_c en funcion de size.							OK	
 * Inicializa todas las casillas de tal forma que sean un puzle resuelto 			OK
 * al azar.
 * Finalmente, mezcla el tablero.													OK
 * 
 * En la implementacion del extra 1 se debe añadir:
 * Pedir al usuario el tipo del tablero (simple o doble), verificando				OK
 * que el tipo introducido sea correcto. 
 * Inicializa los campos max_f y max_c en funcion de size y tipo.					OK
 ...
 *	"\nCasillas a intercambiar (ej: [A0B1]): "				OK
 * Verifica que las dos casillas esten dentro de los rangos permitidos 		OK
 * Si las dos casillas son validas y no son la misma las intercambia.		OK
 *
 * En la implementacion del extra 1 se debe añadir:
 * Al pedir al jugador las casillas a intercambiar, se debe verificar que las 	OK
 ...
 * En la implementacion basica:
 * Dado un tablero simple devuelve si esta resuelto 	OK
 *
...
int check_rangos(char,char,int,int ,t_tablero *tablero);
